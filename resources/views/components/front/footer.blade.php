<footer class="relative bg-gray-50 pt-8 pb-6 fade-in">
    <div class="bottom-auto top-0 left-0 right-0 w-full absolute pointer-events-none overflow-hidden -mt-20"
        style="height: 80px;">
        <svg class="absolute bottom-0 overflow-hidden" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none"
            version="1.1" viewBox="0 0 2560 100" x="0" y="0">
            <polygon class="text-gray-50 fill-current" points="2560 0 2560 100 0 100"></polygon>
        </svg>
    </div>
    <div class="container mx-auto px-4">
        <div class="flex flex-wrap">
            <div class="w-full lg:w-6/12 px-4 mb-4 sm:mb-0 text-center sm:text-left">
                <h4 class="text-3xl font-semibold text-blue-black">Let's keep in touch!</h4>
                <h5 class="text-lg mt-0 mb-2 text-gray-700">
                    Find us on any of these platforms.
                </h5>
                <div class="my-6 text-center sm:text-left">
                    <button
                        class="bg-white text-blue-400 shadow-lg font-normal h-10 w-10 items-center justify-center align-center rounded-full outline-none focus:outline-none mr-2 p-3"
                        type="button">
                        <x-icon.twitter />
                    </button>
                    <button
                        class="bg-white text-blue-600 shadow-lg font-normal h-10 w-10 items-center justify-center align-center rounded-full outline-none focus:outline-none mr-2 p-3"
                        type="button">
                        <x-icon.facebook />
                    </button>
                </div>
            </div>
            <div class="w-full lg:w-6/12 px-4">
                <div class="flex flex-wrap items-top mb-6">
                    <div class="w-full lg:w-4/12 px-4 ml-auto mb-2">
                        <span class="block uppercase text-blue-black text-sm font-bold mb-2">Useful Links</span>
                        <ul class="list-unstyled">
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="{{ url('/').'#about' }}">About Us</a>
                            </li>
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="https://bmywa.com">BMYWA</a>
                            </li>
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="creative-writing">Creative Writing</a>
                            </li>
                        </ul>
                    </div>
                    <div class="w-full lg:w-4/12 px-4">
                        <span class="block uppercase text-blue-black text-sm font-bold mb-2">Other
                            Resources</span>
                        <ul class="list-unstyled">
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="{{ url('/terms') }}">Terms &amp; Conditions</a>
                            </li>
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="{{ url('/privacy') }}">Privacy Policy</a>
                            </li>
                            <li>
                                <a class="text-gray-700 hover:text-blue-black font-semibold block pb-2 text-sm"
                                    href="{{ url('/').'#contact-us' }}">Contact Us</a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <hr class="my-6 border-gray-400">
        <div class="flex flex-wrap items-center md:justify-between justify-center">
            <div class="w-full md:w-4/12 px-4 mx-auto text-center">
                <div class="text-sm text-blue-black font-semibold py-1">
                    Copyright © {{ date('Y') }} BMCCN
                    {{-- <a href="#" class="text-blue-black hover:text-blue-950">GuiltyLogik</a>. --}}
                </div>
            </div>
        </div>
    </div>
</footer>
