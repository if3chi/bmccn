 <div class="container mx-auto p-4" x-data="gallery()">
             ...
             <template x-for="(image, index) in displayedImages" :key="index">
                 <div class="flex-shrink-0 w-64 h-96 relative">
                     <img :src="image" alt="Gallery Image"
                         class="w-full h-full object-cover rounded-lg shadow-md cursor-pointer" @click="openModal(image)">
                 </div>
             </template>
         </div>
     </div>
     ...
     <div class="relative" x-data="carousel()">
         <div class="relative lg:pl-3">
             ...
             <button @click="prev"
                 class="absolute left-0 lg:left-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white px-3 py-2 rounded-full">
     ...
     <!-- Image Modal -->
     <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" x-cloak>
